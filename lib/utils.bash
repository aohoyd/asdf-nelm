...
download_release() {
	...
	gpg --no-default-keyring --keyring "$ringfilename" --import "$keyfilename" || fail "Could not import $keyfilename"
	gpg --no-default-keyring --keyring "$ringfilename" --trust-model always --verify "$sigfilename" "$filename" || fail "Could not verify $filename"

	rm "$ringfilename" "$sigfilename"
}
